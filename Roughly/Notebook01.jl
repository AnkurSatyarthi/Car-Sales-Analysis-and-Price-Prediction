...
png("Output/myplot00")
Plots.pdf(rsbysp, "Output/myplot00")
x = range(0,10,length=100)
y1 = sin.(x)
y2 = cos.(x)
plot(x, [y1,y2], title="Trigonometric functions",label=["sin(x)" "cos(x)"], linewidth=3)
savefig("Output/Trigonometric_function.png")
plot(cars.year, [cars.selling_price, cars.km_driven, cars.fuel], layout=(3,1),legend=false)
# savefig()
fuel_nd_selling_price = scatter(cars.year,cars.selling_price,markersize=[4,5,6])
km_driven_nd_selling_price = scatter(cars.km_driven,cars.selling_price)
histogram(cars.selling_price,cars.year)
histogram(cars.km_driven,cars.year)
histogram2d(cars.selling_price,cars.year)
histogram2d(cars.year,cars.selling_price)
barhist(cars.selling_price,cars.year)
