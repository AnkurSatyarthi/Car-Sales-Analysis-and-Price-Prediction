...
begin
    cars=DataFrame(CSV.File("data/CAR DETAILS FROM CAR DEKHO.csv"));describe(cars,:all);X=round.(Int,cars.km_driven/1000);Y=round.(Int,cars.selling_price/1000);
    ...
    ylabel="PRICE (In thousands of Rupees, ref: CarDekho)",color=:skyblue);t=Table(X=X,Y=Y);gr(size=(600,600));plot_scatter=scatter(X,Y,ma=0.5,ms=5,xlims=(0,850),ylims=(0,8999),xlabel="KM DRIVEN (In thousands)",color=:teal,
